...
Structures bridgebots couvertes :
  ...
  - Rank          → Enum  : TWO … ACE
  - Card          → BaseModel(rank: Rank, suit: Suit)
  - Bid           → BaseModel(level: int, suit: Suit | None)  # None = Sans-Atout
  - BidAnnotation → BaseModel(alert: bool, explanation: str | None)
  - BidRecord     → BaseModel(dealer, bids)
  - BoardRecord   → BaseModel(board_num, dealer, vulnerable, hands, bid_record)
 
Installation :
    pip install bridgebots lark pydantic
...
try:
    ...
    # ── Substituts Pydantic (si bridgebots absent) ────────────────────────────
    ...
    class Card(BaseModel):
        ...
        model_config = {"frozen": True}   # immuable, hashable
 
    class Bid(BaseModel):
        level: int                         # 1–7, ou 0 pour PASS / DOUBLE / REDOUBLE
        suit:  Optional[Suit] = None       # None = Sans-Atout
        call:  Optional[str]  = None       # "PASS" | "X" | "XX" | None
 
    class BidAnnotation(BaseModel):
        alert:       bool
    ...
    # Plages de valeurs raisonnables par type de comptage
    _PT_MAX: dict[str, int] = {"H": 37, "HL": 44, "HLD": 50}
 
    class PtConstraint(BaseModel):
        """
        Contrainte sur les points d'une main, tous systèmes confondus.
 
        Trois types de comptage :
          H   = points Honneurs seuls
                (As=4, Roi=3, Dame=2, Valet=1  —  max 37)
          HL  = H + points de Longueur
                (+1 par carte au-delà de 4 dans une couleur  —  max ~44)
          HLD = HL + points de Distribution
                (chicane=3, singleton=2, doubleton=1  —  max ~50)
 
        Exemples parsés :
          "H >= 12"    → PtConstraint(pt_type="H",   operator=">=", value=12)
          "HL <= 23"   → PtConstraint(pt_type="HL",  operator="<=", value=23)
          "HLD >= 15"  → PtConstraint(pt_type="HLD", operator=">=", value=15)
        """
        pt_type:  str   # "H" | "HL" | "HLD"
        operator: str   # ">=" | "<=" | ">" | "<" | "="
        value:    int
 
        @model_validator(mode="after")
        def check_value(self) -> "PtConstraint":
            max_val = _PT_MAX.get(self.pt_type, 37)
            if not (0 <= self.value <= max_val):
                raise ValueError(
                    f"{self.pt_type} : valeur {self.value} hors plage [0, {max_val}]"
                )
            return self
 
    # Distributions équilibrées reconnues comme "régulières" au bridge
    _REGULAR_DISTRIBUTIONS: frozenset[str] = frozenset({"4333", "4432", "5332"})
 
    class RegularHand(BaseModel):
        """
        Contrainte : la main est régulière (distribution équilibrée).
 
        Une main est régulière si et seulement si sa distribution appartient à :
          4-3-3-3  •  4-4-3-2  •  5-3-3-2
        (pas de chicane, pas de singleton, au plus un doubleton)
 
        Deux formes acceptées par la grammaire :
          Mot-clé  : "REGULIERE" | "REGULAR" | "BALANCED"
                     → RegularHand(distribution=None)
          Patron   : "4333" | "4432" | "5332"  (avec ou sans tirets)
                     → RegularHand(distribution="4333")  # forme normalisée
 
        is_regular vaut toujours True : la règle ne matche que les mains
        régulières. Pour exprimer une main irrégulière, utiliser NOT/NON.
        """
        is_regular:   bool            = True
        distribution: Optional[str]   = None   # "4333" | "4432" | "5332" | None
 
        @model_validator(mode="after")
        def check_distribution(self) -> "RegularHand":
            if self.distribution is not None:
                if self.distribution not in _REGULAR_DISTRIBUTIONS:
                    raise ValueError(
                        f"Distribution '{self.distribution}' non régulière — "
                        f"attendu : {sorted(_REGULAR_DISTRIBUTIONS)}"
                    )
            return self
 
    class BiddingRule(BaseModel):
        """
        Règle d'enchère conditionnelle : BID SI condition (ET condition)*.
 
        Exprime qu'un joueur DOIT annoncer BID si toutes les conditions
        sur sa main sont satisfaites simultanément.
 
        Syntaxe parsée :
          "<enchère> SI <cond> ET <cond> …"
          "<bid> IF <cond> AND <cond> …"
 
        Chaque condition est soit :
          - un PtConstraint  : "H >= 15", "HL <= 23", "HLD >= 17" …
          - un RegularHand   : "REGULIERE", "BALANCED", "4432" …
 
        Exemple :
          "1SA SI REGULIERE ET H >= 15 ET H <= 17"
          → BiddingRule(
                bid        = Bid(level=1, suit=None),       # 1 Sans-Atout
                conditions = [
                    RegularHand(),
                    PtConstraint(pt_type="H", operator=">=", value=15),
                    PtConstraint(pt_type="H", operator="<=", value=17),
                ]
            )
        """
        bid:        Bid
        conditions: list   # list[RegularHand | PtConstraint]
...
BRIDGE_GRAMMAR = r"""
    ...
    // ── Règle d'enchère conditionnelle ───────────────────────────────────────
    // Syntaxe : BID SI/IF condition (ET/AND condition)*
    //
    // Chaque condition est une contrainte de points (pt_expr)
    // ou une contrainte de régularité (regular_hand).
    // pt_expr est utilisé directement (et non pt_constraint) pour éviter
    // toute ambiguïté avec le connecteur RULE_AND de cette règle.
    //
    // Exemple :
    //   "1SA SI REGULIERE ET H >= 15 ET H <= 17"
    bidding_rule  : bid RULE_IF hand_cond (RULE_AND hand_cond)*
    hand_cond     : regular_hand | pt_expr
    RULE_IF       : /SI|IF/i
    RULE_AND      : /ET|AND/i
 
    // ── Main régulière (distribution équilibrée) ─────────────────────────────
    // Une main est régulière si sa distribution est 4-3-3-3, 4-4-3-2 ou 5-3-3-2
    // (aucune chicane, aucun singleton, au plus un doubleton).
    //
    // Deux formes acceptées :
    //   Mot-clé : "REGULIERE" | "REGULAR" | "BALANCED"
    //   Patron  : "4333" | "4432" | "5332"  (tirets optionnels)
    regular_hand  : REG_KW          -> regular_keyword
                  | REG_PATTERN     -> regular_pattern
    REG_KW        : /REGULIER[E]?|REGULAR|BALANCED/i
    REG_PATTERN   : "4-3-3-3" | "4333"
                  | "4-4-3-2" | "4432"
                  | "5-3-3-2" | "5332"
 
    // ── Contraintes de points d'une main ─────────────────────────────────────
    // Trois systèmes de comptage :
    //   H   = Honneurs seuls          (As=4 Roi=3 Dame=2 Valet=1 — max 37)
    //   HL  = H + Longueur            (+1/carte au-delà de 4     — max ~44)
    //   HLD = HL + Distribution       (chicane=3 sgt=2 dbl=1     — max ~50)
    //
    // Syntaxe :  PT_TYPE PT_OP NUMBER
    //   ex : "H >= 12"   "HL <= 23"   "HLD >= 15"
    //
    // Plusieurs contraintes enchaînées avec AND / ET :
    //   "H >= 12 AND H <= 21"   "HL >= 14 ET HL <= 23"
    //
    // IMPORTANT — ordre des alternatives dans PT_TYPE :
    //   "HLD" doit précéder "HL" qui doit précéder "H"
    //   pour que le lexer matche le token le plus long en premier.
    pt_constraint : pt_expr (PT_AND pt_expr)*
    pt_expr       : PT_TYPE PT_OP PT_NUMBER
    PT_TYPE       : "HLD" | "HL" | "H"
    PT_OP         : ">=" | "<=" | ">" | "<" | "="
    PT_NUMBER     : /[0-9]+/
    PT_AND        : /AND|ET/i
...
class BridgeTransformer(Transformer):
    ...
    @v_args(inline=True)
    def hand_cond(self, cond) -> "RegularHand | PtConstraint":
        return cond
 
    def bidding_rule(self, items: list) -> "BiddingRule":
        bid_obj    = items[0]
        # RULE_IF et RULE_AND sont des Token — on ne garde que les contraintes
        conditions = [i for i in items[1:] if not isinstance(i, Token)]
        return BiddingRule(bid=bid_obj, conditions=conditions)
 
    # ── Main régulière ────────────────────────────────────────────────────────
 
    def regular_keyword(self, _items) -> "RegularHand":
        return RegularHand()
 
    @v_args(inline=True)
    def regular_pattern(self, tok: Token) -> "RegularHand":
        # Normalise "4-3-3-3" → "4333" avant validation Pydantic
        normalized = str(tok).replace("-", "")
        return RegularHand(distribution=normalized)
 
    # ── Contraintes de points ────────────────────────────────────────────────
 
    def PT_NUMBER(self, tok: Token) -> int:
        return int(str(tok))
 
    @v_args(inline=True)
    def pt_expr(self, pt_type: Token, op: Token, value: int) -> "PtConstraint":
        return PtConstraint(pt_type=str(pt_type), operator=str(op), value=value)
 
    def pt_constraint(self, items: list) -> list["PtConstraint"]:
        # Filtre les tokens PT_AND (séparateurs) — ne garde que les PtConstraint
        return [item for item in items if isinstance(item, PtConstraint)]
